...
먼저 웹검색이 가능하면 오늘 기준 Pikmin Bloom 공식 뉴스에서 현재 이벤트 버섯/미스터리 버섯에 강한 데코를 확인해.
공식 뉴스에서 "strongest against these mushrooms", "stronger than usual against all types of mushrooms"처럼 안내된 데코를 기준으로 분류해.
...
데코분류 기준:
- 현재시즌: 현재 이벤트에서 새로 등장한 주력 이벤트 데코.
- 시즌서브: 같은 이벤트 버섯에서 보너스를 받는 복각/작년/보조 이벤트 데코.
- 일반: 데코는 있지만 이번 시즌버섯 보너스 대상이 아닌 피크민.
- 비데코: 데코 아이콘이나 장식이 없는 피크민.
- 검색이 불가능한 경우 2026년 6월 기준으로 현재시즌은 Flower Crown/꽃관/화관/플라워 크라운, 시즌서브는 Surfboard Keychain/서프보드 키체인, Puzzle 2023 Summer Memories, Puzzle 2022 Summer Memories로 분류해.
...
function createHeartSlot(row, friendshipId, value = "") {
  ...
  slot.innerHTML = `
    <span>${friendship.label}</span>
    <input class="heart-count" data-heart="${friendship.id}" type="number" inputmode="numeric" min="0" max="100" step="1" value="${value}" placeholder="0" aria-label="${friendship.label} 마리">
